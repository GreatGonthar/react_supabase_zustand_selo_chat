...
import { delImg, delMessage } from "../../lib/sendMessageToSupabase";
import {
	authButton,
	hover,
	links,
...
const MessageItem = ({ message, authUser, changeUser, chatId }) => {
	...
	return (
		<>
				...
				>
					<Box
						sx={{
							...
							// backgroundColor: isOwnMessage ? links : secondaryElement,
							color: "black",
							p: "8px 16px",
							borderRadius: 3,
							ml: 1,
					...
					>
						<MessageContent message={message} color={isOwnMessage ? "black" : secondaryText} />
					</Box>
			...
			<Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
				<MenuItem
					onClick={async () => {
						await delMessage(message.id);
						if (message.imgurl) {
							await delImg(message.imgurl);
							setAnchorEl(null);
						}
					}}
				>
